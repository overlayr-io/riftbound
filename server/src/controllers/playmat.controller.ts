...
/** Catalogue public : fonds officiels + thèmes unis. */
...
export async function addMyPlaymat(req: Request, res: Response, next: NextFunction) {
  try {
    const { variant, imageUrl, storagePath, zoneStyle } = req.body
    res.status(201).json(await service.addMine(req.user!.uid, { variant, imageUrl, storagePath, zoneStyle }))
  } catch (e: unknown) {
    if (e instanceof Error && e.message === 'LIMIT_REACHED') { res.status(409).json({ error: 'Limite de 3 images atteinte pour cette variante' }); return }
    if (e instanceof Error && e.message === 'INVALID') { res.status(400).json({ error: 'Champs manquants' }); return }
    next(e)
  }
}

export async function deleteMyPlaymat(req: Request, res: Response, next: NextFunction) {
  try { res.json(await service.removeMine(req.user!.uid, param(req.params.id))) }
  catch (e: unknown) {
